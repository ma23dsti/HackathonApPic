...
def chargement_donnees():
    ...
    df_donnees_entrees = pd.DataFrame({
        ...
        'valeur': pd.Series(entree['donnees_observees']['en_unite_mesure']).to_numpy().flatten(),  # Convert to NumPy array before flattening
    })
    ...
    for modele in prevision['modeles']:
        ...
        for i in range(n):
            ...
            donnees_prevision.append({
                ...
                'unite mesure': entree['unite_mesure'],
                'format mesure': entree['format_mesure'],
                'valeur': modele['donnees_predites']['en_unite_mesure'][i]  # Updated to access "en_unite_mesure"
            })
